...
import static com.sun.tools.javac.main.OptionName.SQLREPO;
...
import de.so.ma.helperClasses.HelperClassesManager;
...
public class Main {
    ...
    // de.so.ma: added context param, here
    /** Process command line arguments: store all command line options
     *  in `options' table and return all source filenames.
     *  @param flags    The array of command line arguments.
     */
    public Collection<File> processArgs(String[] flags, Context context) { // XXX sb protected
        return processArgs(flags, null, context);
    }

    /**
     * de.so.ma: Added wrapper method for cases when the context param isn't needed.
     * This is the case when the sql repo option isn't set
     * @param flags
     * @return
     */
    public Collection<File> processArgs(String[] flags) { // XXX sb protected
        return processArgs(flags, null, null);
    }
    
    // de.so.ma: added context param, here
    public Collection<File> processArgs(String[] flags, String[] classNames, Context context) { // XXX sb protected
        int ac = 0;
        ...
        // de.so.ma
        // Notice about the compiler
        if (!StateHelper.usingJtreg()) {
        	System.out.println("Using SQL-enabled compiler");
        }
        // initialize repo
        if (options.isSet(SQLREPO) && context != null) {
        	MetaRepo.initializeInstance(options.get(SQLREPO), Log.instance(context));
        }
                       ...
                       Iterable<? extends Processor> processors)
    {
        ...
        try {
            ...
            try {
                files = processArgs(CommandLine.parse(args), classNames, context);
                if (files == null) {
            ...
            // add helper classes
            // tbd: make it nicer
            if (options.isSet(SQLREPO)) {
            	HelperClassesManager helperClassesManager = new HelperClassesManager(fileObjects, context);
                fileObjects = helperClassesManager.withAddedHelperClasses();
            }
