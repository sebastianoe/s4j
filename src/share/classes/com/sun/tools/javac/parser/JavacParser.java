...
//where
    ...
    public JCExpression term3() {
        ...
        case SQL_TOKEN:
        	t = null;
        	S.nextToken();
        	Token separatorToken = S.token();
        	if (!(separatorToken == Token.LBRACKET || separatorToken == Token.LBRACE)) {
        		System.out.println(separatorToken);
        		// tbd: better exception handling
        		throw new RuntimeException("NoBracketToken");
        	}
        	S.nextToken();
        	
        	JCSqlQuery sqlQuery = null;
        	if (S instanceof Scanner) {
        		sqlQuery= JCSqlQuery.createFromTokens(
        				(Scanner) S,
        				this,
        				separatorToken == Token.LBRACE);
        	} else {
        		// tbd: better exception handling
        		throw new RuntimeException("Lexer isn't a Scanner");
        	}
        	accept(RBRACKET);
        	
        	/*
        	 *  sqlQuery has to be passed through to the attribution phase.
        	 *  Here, the actual transformation will happen.
        	 */
        	t = sqlQuery;
